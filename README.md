...
| `REPLY_QUOTE_CHARS` | `60` | How much of a quoted message to inline when resolving a `[回复]`. |
...
- **Reply resolution.** A `reply` segment carries only the quoted message's id — never its text — so `[回复]写在纸上就行周五交` left neither reader nor model able to tell *which* assignment was meant. Each id is resolved through `get_msg` and inlined: `[回复“工程数学作业如下”]写在纸上就行周五交`. Widening the history window does not substitute for this — QQ allows quoting arbitrarily far back, and a measured pair in a live group sat **8.2 days apart**, past any "last N messages" window. Resolution goes one level deep (a reply to a reply stays `[回复]`, so chains cannot recurse), is cached per message id so a popular notice with a dozen replies costs one lookup, spends **no OCR budget** on quoted images, and degrades to a bare `[回复]` when the original was recalled or expired.
- **Attachment names.** The OneBot `file` segment stores the name under `data.file`; there is no `data.name`, so every attachment previously rendered as an empty `[文件:]`. Group notices are frequently "one sentence plus one xlsx", which makes the file name the only real payload — `[文件:党政办发〔2026〕5号--关于2026年中秋、国庆节放假安排的通知.pdf]` says everything the sentence does not.
