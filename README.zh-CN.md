...
**超过一周左右的图基本就取不回来了。** NapCat 只对近期文件留本地副本，腾讯的 CDN 链接也会过期——而且用 `nc_get_rkey` 换个新 rkey 也救不回来，因为过期的是 `fileid` 本身。在这台机器上实测：最近 7 天的图全都能取到，更早的 17 张里有 16 张已经没了。加了 `reverse_order` 之后能拉到的历史变深了，老图从少见变成了常态。

所以超过 `OCR_MAX_AGE_DAYS`（7 天）的图会在任何网络请求之前就被跳过，**而且不占用识别额度**——过期的图本来就识别不出来，让它占一个额度等于挤掉一张还能识别的新图。实测群 1093527660（52 张图，其中 18 张超过 7 天）：

| | 尝试识别 | 成功 | 保留行数 |
| --- | --- | --- | --- |
| 不做年龄判断 | 40（整个额度） | 22 | 28 |
| 做年龄判断 | 34 | **34** | **40** |

多出来的 12 行不是误差。只有图的消息一旦 OCR 失败就退化成 `[图片]`，而这个正好被去噪正则吃掉——那些消息本来是整条从简报里消失的。图全是老图的群在耗时上受益最大：某个群的清洗从 45.5 秒降到 0.0 秒。

### `count` 是上限，不是承诺

`get_group_msg_history` 给的是 NapCat **本地** QQ 数据库里缓存的那一段，不是腾讯服务器上的全部历史。本地没同步过的消息对这个服务来说就是不存在，而活跃群可能一个小时就把整个本地窗口冲完。

这个窗口有多深，取决于 **`reverse_order`**——本服务现在无条件传它。QQ NT 的 `getMsgsIncludeSelf` 第 4 个参数就是它，默认 `false` = 往【新】的方向查。而翻页的锚点恒等于当前已知最老那条，不传的话每页都在问「比最老那条更新的消息」，回来的永远是已经拿到的那批，翻页循环从来没前进过。2026-09-06 实测，群 102942727，锚点 = 它最老那条：

| `reverse_order` | 返回 |
| --- | --- |
| 不传 / `false` | 158 条，`05-30 → 09-06`——就是锚点那批本身 |
| `true` | 1 条，`05-30 15:07`——向老，且已到底 |

向后的请求还会让 QQNT 把更早的消息**物化进本地库**，而且效果会保持。所以冷库时这个参数最关键——刚扫码重登、或跑过风控画像重置之后，普通 `count` 只能拿到那点已物化的量，只有 `reverse_order=true` 能往回捞。同一个 store，冷库状态下的前后对比：

| 群 | 不传 | 传 |
| --- | --- | --- |
| 102942727 | 14 条 / 14.1 天 | **158 条 / 99.1 天** |
| 1058503877 | 30 条 / **1.0 小时** | **299 条 / 98.6 天** |
| 1093527660 | 30 条 / 0.2 天 | **58 条 / 83.3 天** |

这是变深，不是变无限——339 条摊在 98 天里依然稀疏。而且它要花时间：某个有 72 张图的群，拉取 4 秒、清洗 46 秒，绝大部分耗在对早已过期的图片链接反复尝试 OCR 上。不需要整个窗口时，用 `since_days` / `since` 把范围收住。
...
| `OCR_MAX_AGE_DAYS` | `7` | 超过这个天数的图直接跳过，且不占用识别额度。`0` = 不做年龄判断。 |
...
| `REPLY_QUOTE_CHARS` | `60` | 解引用时，引文最多内联多少字。 |
...
- **解引用。** `reply` 段里只有被引用消息的 id，**没有任何原文**，所以以前渲染出来是 `[回复]写在纸上就行周五交`——读简报的人和模型都无法知道说的是哪份作业。现在每个 id 都用 `get_msg` 点查回来内联进去：`[回复“工程数学作业如下”]写在纸上就行周五交`。这件事**不能靠把历史窗口开大**代替：QQ 允许引用任意久远的消息，实测同一个群里就有间隔 **8.2 天**的引用，任何「最近 N 条」的窗口都覆盖不到。解引用只做一层（回复的回复仍是 `[回复]`，避免成链递归），按 message_id 缓存（一条热门通知底下挂十几条回复也只查一次），对引文里的图**不花 OCR 额度**，原消息已撤回或过期时降级成裸 `[回复]`。
- **附件名。** OneBot 的 `file` 段把文件名放在 `data.file`，并没有 `data.name`，所以以前所有附件都渲染成空的 `[文件:]`。群里大量通知就是「一句话 + 一个 xlsx」，文件名往往是唯一的信息载体——`[文件:党政办发〔2026〕5号--关于2026年中秋、国庆节放假安排的通知.pdf]` 说清了那句话没说的部分。
...
- 每个群的可见范围受限于 NapCat 本地库的深度；`reverse_order` 能往回捞，但捞不出腾讯服务器上没漫游下来的部分。
